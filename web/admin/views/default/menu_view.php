<script>
    var get_menu_url = '/admin/menu/get_all_menu';
    var save_url = '/admin/menu/save';
    var del_url = '/admin/menu/del';
</script>
<style type="text/css">
    .tree-node-selected {
...
<div id="tb" style="height:auto;padding-top:5px;width: 100%">
    <div style="margin-bottom:5px">
        <a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="add()">添加子菜单</a>
...
<div id="mainLayout" class="easyui-layout hidden" data-options="fit: true">
    ...
    <div data-options="region:'west',border:false,split:true,iconCls: 'icon-application_view_tile'"
        ...
        <ul id="admin_menu_tree" class="easyui-tree" data-options="fit:true,border:false">
        </ul>
    </div>
    <!--中部 表格-->
                ...
                <div id="mainTab" class="easyui-tabs" data-options="border:false">
                    <div title="基本设置" style="padding:10px">
                        <table cellpadding="5">
                            ...
                            <tr>
                                <td>中文语言名称:</td>
                                <td colspan="2"><input type="text" name="title" class="easyui-textbox"
                                                       data-options="required:true"></td>
                            </tr>
                            <tr>
                                <td>模块名称:</td>
                                <td>
                                    <input class="easyui-textbox" type="text" name="s">
                                </td>
                                <td>操作时的控制器名，例如content.</td>
                            </tr>
                            <tr>
                                <td>控制器名:</td>
                                <td><input class="easyui-textbox" type="text" name="c"></td>
                                <td>操作时的控制器名，例如content.</td>
                            </tr>
                            <tr>
                                <td>方法名称:</td>
                                <td><input class="easyui-textbox" type="text" name="m"></td>
                                <td>操作时方法名称，例如add.</td>
                            </tr>
                            <tr>
                                <td>菜单图标:</td>
                            ...
                            <tr>
                                <td>是否开启自定义权限:</td>
                                <td>
                                    <input type="radio" value="1" name="is_custom"> 是 <input type="radio" value="0"
                                                                                             name="is_custom"> 否
                                </td>
                                <td>选择
                                    <是>，需要在控制器的方法中自己控制权限，会跳过后台统一的权限验证。当选择
                                        <否>，会使用后台统一的权限验证
                                </td>
                            </tr>
                            <tr>
...
<script>
    ...
    $(function () {
        $('#fm').form('load', {is_custom: 0, display: 1, listorder: 50});

        $('#admin_menu_tree').tree({
            title: '',
            url: get_menu_url,
            ...
            onExpand: function (node) {
                store_tree_state('admin_menu_tree');
            },
            onCollapse: function (node) {
                store_tree_state('admin_menu_tree');
            },
            onLoadSuccess: function (node, data) {
                //$('#admin_menu_tree').tree('expandAll');
                restore_tree_state('admin_menu_tree');
                if (select_node_id > 0) {
    ...
    function add() {
        $('#fm').form('clear');
        var row = $('#admin_menu_tree').tree('getSelected');
        if (row) {
    ...
    function del() {
        var row = $('#admin_menu_tree').tree('getSelected');
        var parent = $('#admin_menu_tree').tree('getParent', row.target);
