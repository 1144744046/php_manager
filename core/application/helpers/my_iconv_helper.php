...
function utf8_to_gbk($utfstr) {
	...
	$okstr = '';
	if(empty($UC2GBTABLE)) {
		$filename = ROOTPATH.'static/data/encoding/gb-unicode.table';
...
function big5_to_gbk($Text) {
	global $BIG5_DATA;
	if(empty($BIG5_DATA)) {
		$filename = ROOTPATH.'static/data/encoding/big5-gb.table';
		$fp = fopen($filename, 'rb');
...
function gbk_to_big5($Text) {
	global $GB_DATA;
	if(empty($GB_DATA)) {
		$filename = ROOTPATH.'static/data/encoding/gb-big5.table';
		$fp = fopen($filename, 'rb');
 ...
 * gbk转拼音
 * @param $txt
 * $type string|array 当类型是string返回的数据是组合好的拼音
 * array时返回的数据是每次字拼音组成的数组
 */
function gbk_to_pinyin($txt,$type='string') {
	if(C('charset') != 'gbk') {
		$txt = iconv(C('charset'),'GBK',$txt);
	}
	$l = strlen($txt);
	$i = 0;
	$pyarr = array();
	$py = $type =='string' ? '' : array();
	$filename = ROOTPATH.'static/data/encoding/gb-pinyin.table';
	$fp = fopen($filename,'r');
	while(!feof($fp)) {
